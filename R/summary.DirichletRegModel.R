summary.DirichletRegModel <- function(object, digits=max(3, getOption("digits") - 3), ...) 
{
    ...
    cat("\nCall:\n",
        paste(strwrap(deparse(object$call, width.cutoff=500), .wd), sep="\n", collapse="\n"),
        "\n", sep = "")
    ...
    if(object$parametrization == "common"){
      ...
      for(i in 1:length(object$varnames)){
        cat(paste(rep("-", min(63, .wd)),sep="",collapse=""),sep="",collapse="")
      ...
      cat(paste(rep("-", min(63, .wd)),sep="",collapse=""),"\n",sep="",collapse="")
    ...
    cat("AIC: ",format(AIC(object),digits=digits),", BIC: ",BIC(object),"\n",sep="",collapse="")
