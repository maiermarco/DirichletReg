...
toQuaternary <- function(abcd){
  sqrt3     <- 1.732050807568877293527446341505872366942805253810380628055806979
  sqrt8.3   <- .9428090415820633658677924828064653857131145835846320487844531587
  twothirds <- .6666666666666666666666666666666666666666666666666666666666666667

  return(cbind(
    x = (abcd[,1L] + 2*abcd[,3L] + abcd[,4L]) / sqrt3,
    y = -(abcd[,2L] + abcd[,3L] + twothirds*abcd[,4L]),
    z = sqrt8.3 * abcd[,4L]
  ))

}

toQuaternaryVectors <- function(c1, c2, c3, c4){ return(toTernary(cbind(c1, c2, c3, c4))) }
